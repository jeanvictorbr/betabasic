...
module.exports = async function generatePontoMenu(interaction, settings) {
    ...
    // Verifica se é admin para mostrar o botão extra
    ...
    // Adiciona botão Admin se tiver permissão
    if (isAdmin) {
        components.push(
            { "type": 14, "divider": true, "spacing": 1 },
            {
                "type": 1,
                "components": [
                    { "type": 2, "style": 4, "label": "Painel Admin (Ajustes/Reset)", "emoji": { "name": "🛠️" }, "custom_id": "ponto_open_admin_panel" }
                ]
            }
        );
    }
    ...
    components.push({ "type": 14, "divider": true, "spacing": 1 }, { "type": 10, "content": " ↘   Configure para conseguir ativar" });
