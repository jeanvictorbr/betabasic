// handlers/selects/aut_mass_remove_role_select.js
const { V2_FLAG, EPHEMERAL_FLAG } = require('../../utils/constants');

module.exports = {
    customId: 'aut_mass_remove_role_select',
    async execute(interaction) {
        ...
        const payload = {
            ...
            components: [
                { type: 10, content: `## ⚠️ Confirmação Final\nVocê tem certeza que deseja **REMOVER** o cargo <@&${roleId}> de **TODOS** os \`${membersCount}\` membros do servidor?` },
                { type: 10, content: "Esta ação é irreversível e pode demorar vários minutos." },
                {
                    type: 1,
                    components: [
                        { type: 2, style: 4, label: 'Sim, Remover Cargo', emoji: { name: '➖' }, custom_id: `aut_mass_remove_role_confirm_${roleId}` },
